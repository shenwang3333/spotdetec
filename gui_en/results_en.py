"""Results display (English): summary text and table preview."""
...
from PyQt5.QtWidgets import QWidget, QVBoxLayout, QTextEdit, QTabWidget, QTableWidget, QTableWidgetItem, QPushButton, QMessageBox
...
class ResultsWidgetEN(QWidget):
    def __init__(self, parent: QWidget | None = None) -> None:
        ...
        self.export_btn.setToolTip("CSV/plots are written automatically to the chosen output folder.")
        self.export_btn.clicked.connect(self._on_export)
        layout.addWidget(self.export_btn)

        self._last_output_dir: Path | None = None

    def set_result(self, result: "SingleChannelResult | DualChannelResult", mode: str, output_dir: Path | None = None) -> None:
    ...
    def _on_export(self) -> None:
        if self._last_output_dir is not None and self._last_output_dir.exists():
            QMessageBox.information(self, "Export", f"Results were saved to:\n{self._last_output_dir}")
            return
        QMessageBox.information(self, "Export", "CSV/plots are written automatically to the output folder when you run analysis.")
