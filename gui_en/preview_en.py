"""Preview dialog (English): visualize detected spots overlaid on the image."""
...
class PreviewDialogEN(QDialog):
    def __init__(self, parent: QWidget | None = None) -> None:
        super().__init__(parent)
        self.setWindowTitle("Preview: spot detection overlay")
        self.resize(900, 700)
    ...
    def run_preview(self, image_path: Path, params: PipelineParams) -> None:
        ...
        if len(spots) == 0:
            msg += (
                "\nTips for 0 spots:\n"
                "- Lower Min mass (try 0 first)\n"
                "- Lower Preprocess σ (try 0–1)\n"
                "- Try Diameter 5/7/9 depending on your pixel size\n"
            )
